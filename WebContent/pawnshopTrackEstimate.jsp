		...
		<c:forEach items="${trackMyEstimate}" var="track">
			<c:if test="${track.estimateStatus != 'complete'}">
				<c:if test="${track.estimateStatus != 'approvedenei'}">
					...
					<div class="cd-timeline-block filter-column ${track.estimateStatus}">
						<!-------- icon -------->
						...
						<div class="cd-timeline-content">
							<!-- date -->
							<span class="cd-date">
								<fmt:formatDate pattern="dd" value="${track.pawnerPostId.pawnerPostDate }" />
								...
								<div class="history-show-estimate">
									<div class="row d-flex">
										...
										<div class="col-md-4 ml-auto">
											เสนอราคาที่
											<span style="color: #ff3300; font-weight: bold;">${track.estimatePriceMin}-${track.estimatePriceMax} บาท</span>
										</div>
										<div class="col-md-3">
											<form action="pawnshop-estimate-approve.html" method="post">
												...
												<button class="btn-custom btn-custom-defalt small" type="submit">สินค้าจำนำถึงโรงรับจำนำแล้ว</button>
											</form>
										</div>
									</div>
								</div>
									...
									<div class="row d-flex">
										...
										<div class="col-md-1">
											<form action="pawnshop-estimate-approve.html" method="post">
												<input type="hidden" name="estimateId" value="${track.estimateId}">
												<input type="hidden" name="pawnerPostId" value="${track.pawnerPostId.pawnerPostId}">
												<input type="hidden" name="status" value="approvedenei">
												<button class="btn-custom btn-custom-defalt small" type="submit" style="position: absolute; top: 16px; left: -21px;">ฉันรู้แล้ว</button>
											</form>
										</div>
									</div>
								</div>
							...
							<c:if test="${track.estimateStatus == 'process'}">
								...
								<div class="history-show-estimate">
									<div class="row d-flex">
										...
										<div class="col-md-3">
											สถานะ:
											<span style="color: #ff3300; font-weight: bold;">รอการตอบรับ</span>
										</div>
									</div>
								</div>
							</c:if>

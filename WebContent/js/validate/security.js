(function($, window) {
	...
	$.formUtils.addValidator({
		...
		validatorFunction : function(value, $el, config, language, $form) {
			var conf = "", confInputName = $el.valAttr("confirm")
					|| $el.attr("name") + "_confirmation", confInput = $form
					.find('input[name="' + confInputName + '"]').eq(0);
			if (confInput) {
				conf = confInput.val()
			} else {
			...
			if (allowing.length > 0) {
				...
				$.each(allowing, function(i, cardName) {
					if (cardName in cards) {
						if (value.length >= cards[cardName][0]
								&& value.length <= cards[cardName][1]) {
							hasValidLength = true;
							return false
						}
					} else {
		...
		$
				.ajax({
					...
					error : function(error) {
						alert("Server validation failed due to: "
								+ error.statusText);

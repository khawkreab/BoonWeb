(function($, window) {
	var SUPPORTS_FILE_READER = typeof window.FileReader != "undefined", _getTypes = function(
			$input) {
		var allowedTypes = $.split(($input.valAttr("allowing") || "")
				.toLowerCase());
		if ($.inArray("jpg", allowedTypes) > -1
				&& $.inArray("jpeg", allowedTypes) == -1)
			allowedTypes.push("jpeg");
		else if ($.inArray("jpeg", allowedTypes) > -1
				&& $.inArray("jpg", allowedTypes) == -1)
			...
			.addValidator({
				...
				validatorFunction : function(str, $input) {
					...
					if (SUPPORTS_FILE_READER) {
						var valid = true, mime = "", allowedTypes = _getTypes($input);
						...
						if (!valid) {
							_log("Trying to upload a file with mime type "
									+ mime + " which is not allowed")
						}
						return valid
					} else {
						_log("FileReader not supported by browser, will check file extension");
						return $.formUtils.validators.validate_extension
								.validatorFunction(str, $input)
					}
				},
	...
	$.formUtils.addValidator({
		...
		validatorFunction : function(value, $input) {
			var valid = true, types = _getTypes($input);
			$.each($input.get(0).files || [], function(i, file) {
				var val = file.value, ext = val
						.substr(val.lastIndexOf(".") + 1);

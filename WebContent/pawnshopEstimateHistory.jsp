				...
				<c:if
					...
					<div class="timeline-list filter-column ${post.estimateStatus}">
						...
						<div class="timeline-title">
							<div class="mr-auto">
								<span>เลขที่การจำนำ #</span> <i> ลงจำนำวันที่ <fmt:formatDate
										pattern="dd MMM yyyy" value="${post.estimateAccessDate }" />
								</i>
								<!----- for pawnshop ----->
							...
							<div class="p-2">
								<i> จำนำวันที่ <fmt:formatDate pattern="d MMM yyyy"
										value="${post.estimateAccessDate }" />
								</i> <i style="color: #ff3300; font-weight: bold">
									เสนอราคาไว้ที่ ${post.estimatePriceMin} -
									${post.estimatePriceMax} บาท </i>
							</div>
						</div>
						<line-x></line-x>
						<!-- item -->
						<div class="timeline-detail">
							...
							<div class="timeline-img">
								<img
									src="img/uploadimg/pawnerPost/${post.pawnerPostId.pawnerPostPicture }" />
							</div>
							<!----- detail ------>
							<div>
								...
								<ul>
									...
									<c:if test="${post.pawnerPostId.pawnerPostTypeCamera != null}">
										<li>ชนิดของกล้อง :
											${post.pawnerPostId.pawnerPostTypeCamera}</li>
										<li>ชนิดของเลนกล้อง :
											${post.pawnerPostId.pawnerPostCameraLen}</li>
									</c:if>
									<!-- Gold -->
									...
									<c:if
										test="${post.pawnerPostId.pawnerPostDescription != null && post.pawnerPostId.pawnerPostDescription != ''}">
										<i class="quick small">เพิ่มเติม
											${post.pawnerPostId.pawnerPostDescription }</i>
									</c:if>
								</ul>
							</div>
							<!----- end detail ------>
